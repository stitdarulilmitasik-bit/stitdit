...
class KHSController extends Controller
{
    ...
    public function viewKHS($code)
    {
        ...
        $data['khs'] = KHS::with(['mahasiswa.programStudi.fakultas', 'tahunAkademik', 'nilaiSemester.mataKuliah'])
            ->where('code', $code)
            ->firstOrFail();
    ...
    {
        try {
            $khs = KHS::with(['mahasiswa.programStudi.fakultas', 'tahunAkademik', 'nilaiSemester.mataKuliah'])
                ->where('code', $code)
                ->firstOrFail();
            ...
            $data['khs'] = $khs;
            $data['webs'] = WebSetting::first();
            $data['nilai_semester'] = $khs->nilaiSemester;
    ...
    public function detailKHS($code)
    { return $this->viewKHS($code); }

    public function bulkGenerate(Request $request)
    { return $this->bulkGenerateKHS($request); }
