...
<style>
  ...
  :root{
    ...
    .hero-section {
    background: linear-gradient(135deg, rgba(var(--tblr-info-rgb), 0.85), rgba(var(--tblr-primary-rgb), 0.9)) no-repeat center center;
      ...
      <div class="col-lg-8">
        <div class="row g-3" id="news-container">
          @forelse(($beritas ?? []) as $berita)
            @php
              $thumb = stit_news_image_url($berita->photo);
            @endphp
        ...
        @if(isset($recentBerita) && $recentBerita->count() > 0)
          <div class="side-card mb-3">
            ...
            @foreach($recentBerita as $recent)
              @php $rthumb = stit_news_image_url($recent->photo); @endphp
