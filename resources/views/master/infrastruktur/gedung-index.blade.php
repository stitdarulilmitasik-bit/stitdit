                ...
                <div class="card-body">
                    ...
                    <div class="row mb-4">
                        <div class="col-md-6 mb-2">
                            <div class="p-3 bg-light-primary rounded">
                                <h6 class="mb-2">Total Gedung</h6>
                                <h3 class="mb-0">{{ count($gedungs) }}</h3>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="collapse" id="collapseForm">
                        <div class="card card-body border">
                            ...
                            <form action="{{ route($spref . 'infrastruktur.gedung-handle') }}" method="post" enctype="multipart/form-data">
                                @csrf
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        <label for="name" class="form-label">Nama Gedung</label>
                                        <input type="text" class="form-control" name="name" id="name" placeholder="Contoh: Gedung A" required>
                                        @error('name')
                                    ...
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <input type="text" class="form-control" name="code" id="code" placeholder="Contoh: GDA" required>
                                        @error('code')
                                            <small class="text-danger">{{ $message }}</small>
                                        @enderror
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        <label for="photo" class="form-label">Foto Gedung</label>
                                    ...
                                    <div class="col-12 mb-3">
                                        ...
                                        <textarea name="desc" id="desc" class="form-control" rows="3" placeholder="Deskripsi singkat gedung"></textarea>
                                        @error('desc')
                                            <small class="text-danger">{{ $message }}</small>
                                        @enderror
                                    </div>
                            ...
                            <tbody>
                                @foreach ($gedungs as $key => $item)
                                    <tr>
                                        ...
                                        <td data-label="Foto">
                                            @if($item->photo)
                                                <img src="{{ stit_storage_image_url('images/gedung', $item->photo) }}" alt="{{ $item->name }}" class="img-thumbnail" style="max-width: 100px;">
                                            @else
                                        ...
                                        <td>
                                            <div class="btn-group" role="group">
                                                <a href="#" data-bs-toggle="modal" data-bs-target="#editData{{ $item->code }}" class="btn btn-sm btn-primary" data-bs-toggle="tooltip" title="Edit Gedung">
                                                    <i class="fas fa-edit"></i>
                                                </a>
                                                <form action="{{ route($spref . 'infrastruktur.gedung-delete', $item->code) }}" method="POST" class="d-inline" id="delete-form-{{ $item->code }}">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button" class="btn btn-sm btn-danger" data-confirm-delete="true" data-bs-toggle="tooltip" title="Hapus Gedung" onclick="confirmDelete('{{ $item->code }}')">
                ...
                <div class="card-body">
                    ...
                    <div class="alert alert-light-success">
                        ...
                        <ul class="mb-0">
                            ...
                            <li>Klik ikon <i class="fas fa-edit"></i> untuk mengedit data gedung</li>
                    ...
                    @if(count($gedungs) > 0)
                        <div class="mt-4">
                            ...
                            <div class="list-group">
                                @foreach($gedungs->take(5) as $gedung)
                                    <div class="list-group-item list-group-item-action">
                                        <div class="d-flex w-100 justify-content-between">
                                            <h6 class="mb-1">{{ $gedung->name }}</h6>
                                            <small class="text-muted">{{ $gedung->code }}</small>
                                        </div>
                                        <p class="mb-1">{{ Str::limit($gedung->desc, 50) }}</p>
                                    </div>
                                @endforeach
                            </div>
                        </div>
        ...
        <div class="modal fade" id="editData{{ $item->code }}" tabindex="-1" role="dialog" aria-labelledby="editModalLabel{{ $item->code }}" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                <div class="modal-content">
                    <form action="{{ route($spref . 'infrastruktur.gedung-update', $item->code) }}" method="POST" enctype="multipart/form-data">
                        @method('patch')
                        @csrf
                        <div class="modal-header">
                            <h5 class="modal-title" id="editModalLabel{{ $item->code }}">Edit Gedung - {{ $item->name }}</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    <label for="edit_name{{ $item->code }}" class="form-label">Nama Gedung</label>
                                    <input type="text" class="form-control" name="name" id="edit_name{{ $item->code }}" value="{{ $item->name }}" placeholder="Contoh: Gedung A" required>
                                    @error('name')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label for="edit_code{{ $item->code }}" class="form-label">Kode Gedung</label>
                                    <input type="text" class="form-control" name="code" id="edit_code{{ $item->code }}" value="{{ $item->code }}" placeholder="Contoh: GDA" required>
                                    @error('code')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label for="edit_photo{{ $item->code }}" class="form-label">Foto Gedung</label>
                                    ...
                                    @if($item->photo)
                                        <img src="{{ stit_storage_image_url('images/gedung', $item->photo) }}" id="preview{{ $item->code }}" class="image-preview">
                                    @else
                                ...
                                <div class="col-12 mb-3">
                                    ...
                                    <textarea name="desc" id="edit_desc{{ $item->code }}" class="form-control" rows="3" placeholder="Deskripsi singkat gedung">{{ $item->desc }}</textarea>
                                    @error('desc')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
