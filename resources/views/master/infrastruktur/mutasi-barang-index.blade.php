                ...
                <div class="card-body">
                    ...
                    <div class="row mb-4">
                        <div class="col-md-4 mb-2">
                            <div class="p-3 bg-light-primary rounded">
                                <h6 class="mb-2">Total Mutasi</h6>
                                <h3 class="mb-0">{{ count($mutasiBarangs) }}</h3>
                            </div>
                        </div>
                        <div class="col-md-4 mb-2">
                            <div class="p-3 bg-light-success rounded">
                                <h6 class="mb-2">Mutasi Aktif</h6>
                                <h3 class="mb-0">{{ $mutasiBarangs->where('status', 'Aktif')->count() }}</h3>
                            </div>
                        </div>
                        <div class="col-md-4 mb-2">
                            <div class="p-3 bg-light-warning rounded">
                                <h6 class="mb-2">Mutasi Tidak Aktif</h6>
                                <h3 class="mb-0">{{ $mutasiBarangs->where('status', 'Tidak Aktif')->count() }}</h3>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="collapse" id="collapseForm">
                        <div class="card card-body border">
                            <h5 class="card-title mb-3">Tambah Mutasi Baru</h5>
                            <form action="{{ route($spref . 'infrastruktur.mutasi-barang-handle') }}" method="post" enctype="multipart/form-data">
                                @csrf
                                <div class="row">
                                    ...
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <select class="form-select" name="lokasi_awal" id="lokasi_awal" required>
                                    ...
                                    <div class="col-md-6 mb-3">
                                        <label for="lokasi_akhir" class="form-label">Lokasi Akhir</label>
                                        <select class="form-select" name="lokasi_akhir" id="lokasi_akhir" required>
                    ...
                    <div class="table-responsive mt-3">
                        <table class="table">
                            ...
                            <tbody>
                                @foreach ($mutasiBarangs as $key => $item)
                                    <tr>
                                        ...
                                        <td data-label="Lokasi Awal">
                                            <div class="d-flex flex-column">
                                                <span class="fw-bold">{{ $item->lokasiAwal->name }}</span>
                                                <small class="text-muted">{{ $item->lokasiAwal->gedung->name }}</small>
                                            </div>
                                        </td>
                                        <td data-label="Lokasi Akhir">
                                            <div class="d-flex flex-column">
                                                <span class="fw-bold">{{ $item->lokasiAkhir->name }}</span>
                                                <small class="text-muted">{{ $item->lokasiAkhir->gedung->name }}</small>
                                            </div>
                                        </td>
                                        <td data-label="Jumlah">{{ $item->jumlah }}</td>
                                        <td data-label="Kondisi">
                                            <span class="badge bg-{{ $item->kondisi == 'Baik' ? 'success' : ($item->kondisi == 'Rusak Ringan' ? 'warning' : 'danger') }}">
                                                {{ $item->kondisi }}
                                            </span>
                                        </td>
                                        <td data-label="Status">
                                            <span class="badge bg-{{ $item->status == 'Aktif' ? 'success' : ($item->status == 'Tidak Aktif' ? 'warning' : 'secondary') }}">
                                                {{ $item->status }}
                                            </span>
                                        </td>
                                        <td>
                                            <div class="btn-group" role="group">
                                                <a href="#" data-bs-toggle="modal" data-bs-target="#editData{{ $item->code }}" class="btn btn-sm btn-primary" data-bs-toggle="tooltip" title="Edit Mutasi">
                                                    <i class="fas fa-edit"></i>
                                                </a>
                                                <form action="{{ route($spref . 'infrastruktur.mutasi-barang-delete', $item->code) }}" method="POST" class="d-inline" id="delete-form-{{ $item->code }}">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button" class="btn btn-sm btn-danger" data-confirm-delete="true" data-bs-toggle="tooltip" title="Hapus Mutasi" onclick="confirmDelete('{{ $item->code }}')">
        ...
        <div class="col-lg-4 col-12 mb-2">
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title">Informasi Mutasi</h5>
                </div>
                <div class="card-body">
                    <p>Bagian ini menampilkan informasi umum dan petunjuk terkait pengelolaan Mutasi Barang.</p>
                    
                    <div class="alert alert-light-success">
                        <h6 class="">Petunjuk Penggunaan:</h6>
                        <ul class="mb-0">
                            <li>Klik tombol "Tambah Mutasi" untuk menambahkan mutasi baru</li>
                            <li>Klik ikon <i class="fas fa-edit"></i> untuk mengedit data mutasi</li>
                            <li>Klik ikon <i class="fas fa-trash"></i> untuk menghapus mutasi</li>
                        </ul>
                    </div>
                    
                    @if(count($mutasiBarangs) > 0)
                        <div class="mt-4">
                            <h6>Mutasi Terbaru</h6>
                            <div class="list-group">
                                @foreach($mutasiBarangs->sortByDesc('created_at')->take(3) as $mutasi)
                                    <div class="list-group-item list-group-item-action">
                                        <div class="d-flex w-100 justify-content-between">
                                            <h6 class="mb-1">{{ $mutasi->barang->name }}</h6>
                                            <small class="text-muted">{{ $mutasi->created_at->diffForHumans() }}</small>
                                        </div>
                                        <p class="mb-1">Ke: {{ $mutasi->lokasiAkhir->name }}</p>
                                        <small class="text-muted">{{ $mutasi->kondisi }}</small>
                                    </div>
                                @endforeach
                            </div>
                        </div>
        ...
        <div class="modal fade" id="editData{{ $item->code }}" tabindex="-1" role="dialog" aria-labelledby="editModalLabel{{ $item->code }}" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                <div class="modal-content">
                    <form action="{{ route($spref . 'infrastruktur.mutasi-barang-update', $item->code) }}" method="POST" enctype="multipart/form-data">
                        @method('patch')
                        @csrf
                        <div class="modal-header">
                            <h5 class="modal-title" id="editModalLabel{{ $item->code }}">Edit Mutasi - {{ $item->barang->name }}</h5>
                        ...
                        <div class="modal-body">
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-3">
                                    <label for="edit_lokasi_awal{{ $item->code }}" class="form-label">Lokasi Awal</label>
                                    <select class="form-select" name="lokasi_awal" id="edit_lokasi_awal{{ $item->code }}" required>
                                        <option value="">Pilih Lokasi Awal</option>
                                        @foreach ($ruangs as $ruang)
                                            <option value="{{ $ruang->id }}" {{ $item->lokasi_awal == $ruang->id ? 'selected' : '' }}>{{ $ruang->name }} ({{ $ruang->gedung->name }})</option>
                                        @endforeach
                                    </select>
                                    @error('lokasi_awal')
                                        <small class="text-danger">{{ $message }}</small>
                                ...
                                <div class="col-md-6 mb-3">
                                    <label for="edit_lokasi_akhir{{ $item->code }}" class="form-label">Lokasi Akhir</label>
                                    <select class="form-select" name="lokasi_akhir" id="edit_lokasi_akhir{{ $item->code }}" required>
                                        <option value="">Pilih Lokasi Akhir</option>
                                        @foreach ($ruangs as $ruang)
                                            <option value="{{ $ruang->id }}" {{ $item->lokasi_akhir == $ruang->id ? 'selected' : '' }}>{{ $ruang->name }} ({{ $ruang->gedung->name }})</option>
                                        @endforeach
                                    </select>
                                    @error('lokasi_akhir')
                                        <small class="text-danger">{{ $message }}</small>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="number" class="form-control" name="jumlah" id="edit_jumlah{{ $item->code }}" value="{{ $item->jumlah }}" required min="1">
                                    @error('jumlah')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    @if($item->photo)
                                        <img src="{{ stit_storage_image_url('images/mutasi', $item->photo) }}" id="preview{{ $item->code }}" class="image-preview">
                                    @else
    ...
    <script>
        ...
        function confirmDelete(code) {
            Swal.fire({
                ...
                text: "Data mutasi yang dihapus tidak dapat dikembalikan!",
                icon: 'warning',
