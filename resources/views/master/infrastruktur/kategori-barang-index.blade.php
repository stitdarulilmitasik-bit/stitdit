    ...
    <div class="row">
                ...
                <div class="card-body">
                    ...
                    <div class="row mb-4">
                        <div class="col-md-6 mb-2">
                            <div class="p-3 bg-light-primary rounded">
                                ...
                                <h3 class="mb-0">{{ count($kategoriBarangs) }}</h3>
                            </div>
                        </div>
                        <div class="col-md-6 mb-2">
                            <div class="p-3 bg-light-info rounded">
                                <h6 class="mb-2">Kategori Terbaru</h6>
                                <h3 class="mb-0">{{ $kategoriBarangs->sortByDesc('created_at')->take(1)->count() }}</h3>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="collapse" id="collapseForm">
                        <div class="card card-body border">
                            ...
                            <form action="{{ route($spref . 'infrastruktur.kategori-barang-handle') }}" method="post">
                                @csrf
                                <div class="row">
                    ...
                    <div class="table-responsive mt-3">
                        <table class="table">
                            ...
                            <tbody>
                                @foreach ($kategoriBarangs as $key => $item)
                                    <tr>
                                        ...
                                        <td>
                                            <div class="btn-group" role="group">
                                                <a href="#" data-bs-toggle="modal" data-bs-target="#editData{{ $item->code }}" class="btn btn-sm btn-primary" data-bs-toggle="tooltip" title="Edit Kategori">
                                                    <i class="fas fa-edit"></i>
                                                </a>
                                                <form action="{{ route($spref . 'infrastruktur.kategori-barang-delete', $item->code) }}" method="POST" class="d-inline" id="delete-form-{{ $item->code }}">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button" class="btn btn-sm btn-danger" data-confirm-delete="true" data-bs-toggle="tooltip" title="Hapus Kategori" onclick="confirmDelete('{{ $item->code }}')">
        ...
        <div class="col-lg-4 col-12 mb-2">
            <div class="card">
                ...
                <div class="card-body">
                    <p>Bagian ini menampilkan informasi umum dan petunjuk terkait pengelolaan Kategori Barang.</p>
                    
                    <div class="alert alert-light-success">
                        <h6 class="">Petunjuk Penggunaan:</h6>
                        <ul class="mb-0">
                            <li>Klik tombol "Tambah Kategori" untuk menambahkan kategori baru</li>
                            <li>Klik ikon <i class="fas fa-edit"></i> untuk mengedit data kategori</li>
                            <li>Klik ikon <i class="fas fa-trash"></i> untuk menghapus kategori</li>
                        </ul>
                    </div>
                    
                    @if(count($kategoriBarangs) > 0)
                        <div class="mt-4">
                            <h6>Kategori Terbaru</h6>
                            <div class="list-group">
                                @foreach($kategoriBarangs->sortByDesc('created_at')->take(3) as $kategori)
                                    <div class="list-group-item list-group-item-action">
                                        <div class="d-flex w-100 justify-content-between">
                                            <h6 class="mb-1">{{ $kategori->name }}</h6>
                                            <small class="text-muted">{{ $kategori->created_at->diffForHumans() }}</small>
                                        </div>
                                        <p class="mb-1">{{ Str::limit($kategori->desc, 50) }}</p>
                                        <small class="text-muted">{{ $kategori->code }}</small>
                                    </div>
                                @endforeach
                            </div>
                        </div>
        ...
        <div class="modal fade" id="editData{{ $item->code }}" tabindex="-1" role="dialog" aria-labelledby="editModalLabel{{ $item->code }}" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                <div class="modal-content">
                    <form action="{{ route($spref . 'infrastruktur.kategori-barang-update', $item->code) }}" method="POST">
                        @method('patch')
                        @csrf
                        <div class="modal-header">
                            <h5 class="modal-title" id="editModalLabel{{ $item->code }}">Edit Kategori - {{ $item->name }}</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    <label for="edit_name{{ $item->code }}" class="form-label">Nama Kategori</label>
                                ...
                                <div class="col-md-6 mb-3">
                                    <label for="edit_code{{ $item->code }}" class="form-label">Kode Kategori</label>
                                    <input type="text" class="form-control" name="code" id="edit_code{{ $item->code }}" value="{{ $item->code }}" required>
                                    @error('code')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
