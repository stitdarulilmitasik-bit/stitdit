    ...
    <div class="row">
                ...
                <div class="card-body">
                    ...
                    <div class="row mb-4">
                        <div class="col-md-4 mb-2">
                            <div class="p-3 bg-light-primary rounded">
                                <h6 class="mb-2">Total Saldo</h6>
                                <h3 class="mb-0">{{ count($saldos) }}</h3>
                            </div>
                        </div>
                        <div class="col-md-4 mb-2">
                            <div class="p-3 bg-light-success rounded">
                                <h6 class="mb-2">Pemasukan</h6>
                                <h3 class="mb-0">{{ $saldos->where('type', 'Pemasukan')->count() }}</h3>
                            </div>
                        </div>
                        <div class="col-md-4 mb-2">
                            <div class="p-3 bg-light-warning rounded">
                                <h6 class="mb-2">Pengeluaran</h6>
                                <h3 class="mb-0">{{ $saldos->where('type', 'Pengeluaran')->count() }}</h3>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="collapse" id="collapseForm">
                        <div class="card card-body border">
                            <h5 class="card-title mb-3">Tambah Saldo Baru</h5>
                            <form action="{{ route($spref . 'keuangan.saldo-handle') }}" method="post">
                                @csrf
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        <label for="type" class="form-label">Tipe Saldo</label>
                                    ...
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <input type="text" class="form-control" name="transaction_code" id="transaction_code" required>
                                    ...
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <select class="form-select" name="tagihan_id" id="tagihan_id">
                                    ...
                                    <div class="col-12 mb-3">
                                        ...
                                        <textarea name="desc" id="desc" class="form-control" rows="3" placeholder="Deskripsi transaksi"></textarea>
                                        @error('desc')
                                            <small class="text-danger">{{ $message }}</small>
                                        @enderror
                                    </div>
                            ...
                            <tbody>
                                @foreach ($saldos as $key => $item)
                                    <tr>
                                        ...
                                        <td data-label="Tipe">
                                            <span class="badge {{ $item->type == 'Pemasukan' ? 'bg-light-success text-success' : 'bg-light-warning text-warning' }}">
                                                {{ $item->type }}
                                            </span>
                                        </td>
                                        <td data-label="Jumlah">
                                            <div class="d-flex flex-column">
                                                <span class="fw-bold">Rp {{ number_format($item->amount, 0, ',', '.') }}</span>
                                                <small class="text-muted">{{ $item->source }}</small>
                                            </div>
                                        </td>
                                        <td data-label="Status">
                                            <span class="badge {{ $item->status == 'Sukses' ? 'bg-light-success text-success' : ($item->status == 'Pending' ? 'bg-light-warning text-warning' : 'bg-light-danger text-danger') }}">
                                                {{ $item->status }}
                                            </span>
                                        </td>
                                        <td>
                                            <div class="btn-group" role="group">
                                                <a href="#" data-bs-toggle="modal" data-bs-target="#editData{{ $item->code }}" class="btn btn-sm btn-primary" data-bs-toggle="tooltip" title="Edit Saldo">
                                                    <i class="fas fa-edit"></i>
                                                </a>
                                                <form action="{{ route($spref . 'keuangan.saldo-delete', $item->code) }}" method="POST" class="d-inline" id="delete-form-{{ $item->code }}">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button" class="btn btn-sm btn-danger" data-confirm-delete="true" data-bs-toggle="tooltip" title="Hapus Saldo" onclick="confirmDelete('{{ $item->code }}')">
        ...
        <div class="col-lg-4 col-12 mb-2">
            <div class="card">
                ...
                <div class="card-body">
                    <p>Bagian ini menampilkan informasi umum dan petunjuk terkait pengelolaan Saldo.</p>
                    
                    <div class="alert alert-light-success">
                        <h6 class="">Petunjuk Penggunaan:</h6>
                        <ul class="mb-0">
                            <li>Klik tombol "Tambah Saldo" untuk menambahkan transaksi baru</li>
                            <li>Klik ikon <i class="fas fa-edit"></i> untuk mengedit data transaksi</li>
                            <li>Klik ikon <i class="fas fa-trash"></i> untuk menghapus transaksi</li>
                        </ul>
                    </div>
                    ...
                    @if(count($saldos) > 0)
                        <div class="mt-4">
                            ...
                            <div class="list-group">
                                @foreach($saldos->take(5) as $recent)
                                    <div class="list-group-item list-group-item-action">
                                        <div class="d-flex w-100 justify-content-between">
                                            <h6 class="mb-1">{{ $recent->code }}</h6>
                                            <small class="text-muted">{{ $recent->created_at->diffForHumans() }}</small>
                                        </div>
                                        <p class="mb-1">
                                            <span class="badge {{ $recent->type == 'Pemasukan' ? 'bg-light-success text-success' : 'bg-light-warning text-warning' }}">
                                                {{ $recent->type }}
                                            </span>
                                            Rp {{ number_format($recent->amount, 0, ',', '.') }}
                                        </p>
                                        <small class="text-muted">{{ $recent->source }}</small>
                                    </div>
                                @endforeach
                            </div>
                        </div>
        ...
        <div class="modal fade" id="editData{{ $item->code }}" tabindex="-1" role="dialog" aria-labelledby="editModalLabel{{ $item->code }}" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                <div class="modal-content">
                    <form action="{{ route($spref . 'keuangan.saldo-update', $item->code) }}" method="POST">
                        @method('patch')
                        @csrf
                        <div class="modal-header">
                            <h5 class="modal-title" id="editModalLabel{{ $item->code }}">Edit Saldo - {{ $item->code }}</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    <label for="edit_type{{ $item->code }}" class="form-label">Tipe Saldo</label>
                                ...
                                <div class="col-md-6 mb-3">
                                    <label for="edit_amount{{ $item->code }}" class="form-label">Jumlah</label>
                                    <input type="number" class="form-control" name="amount" id="edit_amount{{ $item->code }}" value="{{ $item->amount }}" min="0" required>
                                    @error('amount')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label for="edit_source{{ $item->code }}" class="form-label">Sumber Dana</label>
                                    <input type="text" class="form-control" name="source" id="edit_source{{ $item->code }}" value="{{ $item->source }}" required>
                                    @error('source')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label for="edit_transaction_code{{ $item->code }}" class="form-label">Kode Transaksi </label>
                                    <input type="text" class="form-control" name="transaction_code" id="edit_transaction_code{{ $item->code }}" value="{{ $item->transaction_code }}" required>
                                    @error('source')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
                                ...
                                <div class="col-md-6 mb-3">
                                    <label for="edit_tagihan_id{{ $item->code }}" class="form-label">Tagihan (Opsional)</label>
                                    <select class="form-select" name="tagihan_id" id="edit_tagihan_id{{ $item->code }}">
                                        <option value="">Pilih Tagihan</option>
                                        {{-- Options will be dynamically loaded --}}
                                    </select>
                                    @error('tagihan_id')
