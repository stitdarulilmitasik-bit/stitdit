...
<div class="col-md-3"><select class="form-select" id="filterStatus" onchange="filterTable()"><option value="">Semua Status</option><option value="draft">Draft</option><option value="submitted">Diajukan</option><option value="approved">Disetujui</option><option value="rejected">Ditolak</option><option value="published">Dipublish</option><option value="locked">Dikunci</option></select></div>
...
@php $statusColors=['draft'=>'secondary','submitted'=>'warning','approved'=>'success','rejected'=>'danger','published'=>'primary','locked'=>'dark'];$statusLabels=['draft'=>'Draft','submitted'=>'Diajukan','approved'=>'Disetujui','rejected'=>'Ditolak','published'=>'Dipublish','locked'=>'Dikunci']; @endphp
<span class="badge bg-{{ $statusColors[$krs->status] ?? 'secondary' }}">{{ $statusLabels[$krs->status] ?? ucfirst($krs->status) }}</span>
</td>
...
@if(in_array($krs->status,['draft','submitted']))<button class="btn btn-sm btn-warning" onclick="editKRS('{{ $krs->code }}')" title="Edit"><i class="fas fa-edit"></i></button>@endif
@if($krs->status=='submitted')<button class="btn btn-sm btn-success" onclick="approveKRS('{{ $krs->code }}')" title="Approve"><i class="fas fa-check"></i></button><button class="btn btn-sm btn-danger" onclick="rejectKRS('{{ $krs->code }}')" title="Reject"><i class="fas fa-times"></i></button>@endif
...
@if($krs->status=='published')<a href="{{ route($spref . 'akademik.krs-print',$krs->code) }}" class="btn btn-sm btn-secondary" target="_blank" title="Cetak"><i class="fas fa-print"></i></a><button class="btn btn-sm btn-dark" onclick="lockKRS('{{ $krs->code }}')" title="Kunci"><i class="fas fa-lock"></i></button>@endif
