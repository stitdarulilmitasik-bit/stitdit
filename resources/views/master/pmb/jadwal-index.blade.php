    ...
    <div class="row">
                ...
                <div class="card-body">
                    ...
                    <div class="row mb-4">
                        <div class="col-md-6 mb-2">
                            <div class="p-3 bg-light-primary rounded">
                                ...
                                <h3 class="mb-0">{{ count($jadwals) }}</h3>
                            </div>
                        </div>
                        <div class="col-md-6 mb-2">
                    ...
                    <div class="collapse" id="collapseForm">
                        <div class="card card-body border">
                            <h5 class="card-title mb-3">Tambah Jadwal Baru</h5>
                            <form action="{{ route($spref . 'pmb.jadwal-handle') }}" method="post">
                                @csrf
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        <label for="name" class="form-label">Nama Jadwal</label>
                                        <input type="text" class="form-control" name="name" id="name" placeholder="Contoh: Jadwal Tes Tulis">
                                        @error('name')
                                            <small class="text-danger">{{ $message }}</small>
                                        @enderror
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        <label for="type" class="form-label">Tipe Jadwal</label>
                                    ...
                                    <div class="col-md-6 mb-3">
                                        <label for="gelombang_id" class="form-label">Gelombang Pendaftaran</label>
                                    ...
                                    <div class="col-12 mb-3">
                                        ...
                                        <textarea name="desc" id="desc" class="form-control" rows="3" placeholder="Deskripsi singkat jadwal PMB"></textarea>
                                        @error('desc')
                                            <small class="text-danger">{{ $message }}</small>
                                        @enderror
                                    </div>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="btn-group" role="group">
                                                ...
                                                <form action="{{ route($spref . 'pmb.jadwal-delete', $item->code) }}" method="POST" class="d-inline" id="delete-form-{{ $item->code }}">
                                                    @csrf
                                                    @method('DELETE')
        ...
        <div class="col-lg-4 col-12 mb-2">
            <div class="card">
                ...
                <div class="card-body">
                    <p>Bagian ini menampilkan informasi umum dan petunjuk terkait pengelolaan Jadwal PMB.</p>
                    
                    <div class="alert alert-light-success">
                        <h6 class="">Petunjuk Penggunaan:</h6>
                        <ul class="mb-0">
                            <li>Klik tombol "Tambah Jadwal" untuk menambahkan jadwal baru</li>
                                ...
                                @foreach($jadwals->take(5) as $jadwal)
                                    <div class="list-group-item list-group-item-action">
                                        <div class="d-flex w-100 justify-content-between">
                                            <h6 class="mb-1">{{ $jadwal->name }}</h6>
                                            <small class="text-muted">{{ $jadwal->type }}</small>
                                        </div>
                                        <p class="mb-1">
                                            {{ \Carbon\Carbon::parse($jadwal->start_date)->format('d/m/Y') }} - 
                                            {{ \Carbon\Carbon::parse($jadwal->ended_date)->format('d/m/Y') }}
                                        </p>
                                    </div>
                                @endforeach
                            </div>
        ...
        <div class="modal fade" id="editData{{ $item->code }}" tabindex="-1" role="dialog" aria-labelledby="editModalLabel{{ $item->code }}" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                <div class="modal-content">
                    <form action="{{ route($spref . 'pmb.jadwal-update', $item->code) }}" method="POST">
                        @method('patch')
                        @csrf
                        <div class="modal-header">
                            <h5 class="modal-title" id="editModalLabel{{ $item->code }}">Edit Jadwal - {{ $item->name }}</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    <label for="edit_name{{ $item->code }}" class="form-label">Nama Jadwal</label>
                                    <input type="text" class="form-control" name="name" id="edit_name{{ $item->code }}" value="{{ $item->name }}" placeholder="Contoh: Jadwal Tes Tulis">
                                    @error('name')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label for="edit_type{{ $item->code }}" class="form-label">Tipe Jadwal</label>
                                    <select class="form-select" name="type" id="edit_type{{ $item->code }}">
                                        <option value="">Pilih Tipe</option>
                                        <option value="Pendaftaran" {{ $item->type == 'Pendaftaran' ? 'selected' : '' }}>Pendaftaran</option>
                                        <option value="Tes" {{ $item->type == 'Tes' ? 'selected' : '' }}>Tes</option>
                                        <option value="Wawancara" {{ $item->type == 'Wawancara' ? 'selected' : '' }}>Wawancara</option>
                                        <option value="Pengumuman" {{ $item->type == 'Pengumuman' ? 'selected' : '' }}>Pengumuman</option>
                                        <option value="Daftar Ulang" {{ $item->type == 'Daftar Ulang' ? 'selected' : '' }}>Daftar Ulang</option>
                                    </select>
                                    @error('type')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                ...
                                <div class="col-md-6 mb-3">
                                    <label for="edit_gelombang_id{{ $item->code }}" class="form-label">Gelombang Pendaftaran</label>
                                    <select class="form-select" name="gelombang_id" id="edit_gelombang_id{{ $item->code }}">
                                        <option value="">Pilih Gelombang</option>
                                        @foreach ($gelombangs as $gelombang)
                                            <option value="{{ $gelombang->id }}" {{ $item->gelombang_id == $gelombang->id ? 'selected' : '' }}>{{ $gelombang->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('gelombang_id')
                                        <small class="text-danger">{{ $message }}</small>
                                ...
                                <div class="col-12 mb-3">
                                    ...
                                    <textarea name="desc" id="edit_desc{{ $item->code }}" class="form-control" rows="3" placeholder="Deskripsi singkat jadwal PMB">{{ $item->desc }}</textarea>
                                    @error('desc')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
