                ...
                <div class="card-body">
                    ...
                    <div class="row mb-4">
                        <div class="col-md-6 mb-2">
                            <div class="p-3 bg-light-primary rounded">
                                <h6 class="mb-2">Total Gelombang</h6>
                                <h3 class="mb-0">{{ count($gelombangs) }}</h3>
                            </div>
                        </div>
                        <div class="col-md-6 mb-2">
                    ...
                    <div class="collapse" id="collapseForm">
                        <div class="card card-body border">
                            <h5 class="card-title mb-3">Tambah Gelombang Baru</h5>
                            <form action="{{ route($spref . 'pmb.gelombang-handle') }}" method="post">
                                @csrf
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        <label for="name" class="form-label">Nama Gelombang</label>
                                        <input type="text" class="form-control" name="name" id="name" placeholder="Contoh: Gelombang 1">
                                        @error('name')
                                    ...
                                    <div class="col-12 mb-3">
                                        ...
                                        <textarea name="desc" id="desc" class="form-control" rows="3" placeholder="Deskripsi singkat gelombang pendaftaran"></textarea>
                                        @error('desc')
                                            <small class="text-danger">{{ $message }}</small>
                                        @enderror
                                    </div>
                    ...
                    <div class="table-responsive mt-3">
                        <table class="table">
                            ...
                            <tbody>
                                @foreach ($gelombangs as $key => $item)
                                    <tr>
                                        <td data-label="No">{{ ++$key }}</td>
                                        <td data-label="Nama Gelombang">
                                        ...
                                        <td data-label="Periode">
                                            ...
                                            {{ \Carbon\Carbon::parse($item->ended_date)->format('d/m/Y') }}
                                        </td>
                                        <td>
                                            <div class="btn-group" role="group">
                                                <a href="#" data-bs-toggle="modal" data-bs-target="#editData{{ $item->code }}" class="btn btn-sm btn-primary" data-bs-toggle="tooltip" title="Edit Gelombang">
                                                    <i class="fas fa-edit"></i>
                                                </a>
                                                <form action="{{ route($spref . 'pmb.gelombang-delete', $item->code) }}" method="POST" class="d-inline" id="delete-form-{{ $item->code }}">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button" class="btn btn-sm btn-danger" data-confirm-delete="true" data-bs-toggle="tooltip" title="Hapus Gelombang" onclick="confirmDelete('{{ $item->code }}')">
        ...
        <div class="col-lg-4 col-12 mb-2">
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title">Informasi Gelombang</h5>
                </div>
                <div class="card-body">
                    <p>Bagian ini menampilkan informasi umum dan petunjuk terkait pengelolaan Gelombang Pendaftaran.</p>
                    
                    <div class="alert alert-light-success">
                        <h6 class="">Petunjuk Penggunaan:</h6>
                        <ul class="mb-0">
                            <li>Klik tombol "Tambah Gelombang" untuk menambahkan gelombang baru</li>
                            <li>Klik ikon <i class="fas fa-edit"></i> untuk mengedit data gelombang</li>
                            <li>Klik ikon <i class="fas fa-trash"></i> untuk menghapus gelombang</li>
                        </ul>
                    </div>
                    
                    @if(count($gelombangs) > 0)
                        <div class="mt-4">
                            <h6>Gelombang Terbaru</h6>
                            <div class="list-group">
                                @foreach($gelombangs->take(5) as $gelombang)
                                    <div class="list-group-item list-group-item-action">
                                        <div class="d-flex w-100 justify-content-between">
                                            <h6 class="mb-1">{{ $gelombang->name }}</h6>
                                            <small class="text-muted">{{ $gelombang->jalur->name }}</small>
                                        </div>
                                        <p class="mb-1">
                                            {{ \Carbon\Carbon::parse($gelombang->start_date)->format('d/m/Y') }} - 
                                            {{ \Carbon\Carbon::parse($gelombang->ended_date)->format('d/m/Y') }}
                                        </p>
                                    </div>
                                @endforeach
                            </div>
        ...
        <div class="modal fade" id="editData{{ $item->code }}" tabindex="-1" role="dialog" aria-labelledby="editModalLabel{{ $item->code }}" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                <div class="modal-content">
                    <form action="{{ route($spref . 'pmb.gelombang-update', $item->code) }}" method="POST">
                        @method('patch')
                        @csrf
                        <div class="modal-header">
                            <h5 class="modal-title" id="editModalLabel{{ $item->code }}">Edit Gelombang - {{ $item->name }}</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    <label for="edit_name{{ $item->code }}" class="form-label">Nama Gelombang</label>
                                    <input type="text" class="form-control" name="name" id="edit_name{{ $item->code }}" value="{{ $item->name }}" placeholder="Contoh: Gelombang 1">
                                    @error('name')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
                                ...
                                <div class="col-12 mb-3">
                                    ...
                                    <textarea name="desc" id="edit_desc{{ $item->code }}" class="form-control" rows="3" placeholder="Deskripsi singkat gelombang pendaftaran">{{ $item->desc }}</textarea>
                                    @error('desc')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
    ...
    <script>
        ...
        function confirmDelete(code) {
            Swal.fire({
                ...
                text: "Data gelombang yang dihapus tidak dapat dikembalikan!",
                icon: 'warning',
