    ...
    <div class="row">
                ...
                <div class="card-body">
                    ...
                    <div class="row mb-4">
                        <div class="col-md-6 mb-2">
                            <div class="p-3 bg-light-primary rounded">
                                ...
                                <h3 class="mb-0">{{ $kategori ? count($kategori) : 0 }}</h3>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="collapse" id="collapseForm">
                        <div class="card card-body border">
                            ...
                            <form action="{{ route($spref . 'publikasi.kategori-handle') }}" method="post">
                                @csrf
                                <div class="row">
                            ...
                            <tbody>
                                @foreach ($kategori as $key => $item)
                                    <tr>
                                        ...
                                        <td data-label="Deskripsi">{{ Str::limit($item->desc, 50) }}</td>
                                        <td>
                                            <div class="btn-group" role="group">
                                                ...
                                                <form action="{{ route($spref . 'publikasi.kategori-delete', $item->code) }}" method="POST" class="d-inline" id="delete-form-{{ $item->code }}">
                                                    @csrf
                                                    @method('DELETE')
        ...
        <div class="col-lg-4 col-12 mb-2">
            <div class="card">
                ...
                <div class="card-body">
                    <p>Bagian ini menampilkan informasi umum dan petunjuk terkait pengelolaan Kategori Galeri.</p>
                    ...
                    @if(count($kategori) > 0)
                        <div class="mt-4">
                            ...
                            <div class="list-group">
                                @foreach($kategori->sortByDesc('created_at')->take(3) as $kat)
                                    <div class="list-group-item list-group-item-action">
                                        <div class="d-flex w-100 justify-content-between">
                                            <h6 class="mb-1">{{ $kat->name }}</h6>
                                            <small class="text-muted">{{ $kat->created_at->diffForHumans() }}</small>
                                        </div>
                                        <p class="mb-1">{{ Str::limit($kat->desc, 50) }}</p>
                                    </div>
                                @endforeach
                            </div>
                        </div>
        ...
        <div class="modal fade" id="editData{{ $item->code }}" tabindex="-1" role="dialog" aria-labelledby="editModalLabel{{ $item->code }}" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <form action="{{ route($spref . 'publikasi.kategori-update', $item->code) }}" method="POST">
                        @method('patch')
                        @csrf
                        ...
                        <div class="modal-body">
                            <div class="row">
                                ...
                                <div class="col-md-12 mb-3">
                                    ...
                                    <textarea class="form-control" name="desc" id="edit_desc{{ $item->code }}" rows="3" required>{{ $item->desc }}</textarea>
                                    @error('desc')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
